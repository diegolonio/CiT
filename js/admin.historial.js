var formulario_filtros = document.getElementById('filtros'),
	panel = document.getElementById('panel');

var docs, no, date, filtro_estado, boleta;
...
function cargar_registros(e){
	...
	estado = formulario_filtros.filtro_estado.value;
	if(e.type == 'input'){
		var parametros = 'docs=' + docs + '&boleta=' + boleta + '&no=' + no + '&date=' + date + '&estado=' + estado;
	}else if(e.type == 'load'){
		var parametros = 'pagina=' + pagina() + '&docs=' + docs + '&no=' + no + '&date=' + date + '&estado=' + estado;
	}
		...
		if(!respuesta_registros.error){
			...
			if( !respuesta_registros.registros ){
				...
				for(var tupla = 0; tupla < respuesta_registros.registros.length; tupla++){
					...
					boleta.innerHTML = respuesta_registros.registros[tupla].boleta;
					registro.appendChild(boleta);
